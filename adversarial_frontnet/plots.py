...
def plot_results(path, targets):
    ...
    all_sf, all_tx, all_ty = np.load(path / 'positions.npy')
    # print(all_sf.shape)
    # print(all_tx.shape)
    # print(all_ty.shape)
    ...
    boxplot_data = np.load(path / 'boxplots.npy')
    ...
    with PdfPages(Path(path) / 'result.pdf') as pdf:
        ...
        # else:
        ax.set_title('Loss patch optimization for all iterations')
        ...
        for target_idx, target in enumerate(targets):
            ...
            ax.set_title(f'boxplots for y, patches placed at optimal position for target {target}')
            ax.set_ylabel('y')
...
if __name__=="__main__":
    ...
    target = [[1.5, 2.0, 0.3], [0.7, -2.0, -0.4]]
    plot_results(path, target)